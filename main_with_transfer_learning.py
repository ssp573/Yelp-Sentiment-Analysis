...
train_tokenized_transfer['sentiment']=np.where(train_tokenized['sentiment']=='pos',1,0)
print(train_tokenized.sentiment.unique())
...
train_transfer_tuples= [tuple(x) for x in train_tokenized_transfer[['text','sentiment']].values]
train_transfer_data_tokens,train_transfer_data_labels=zip(*train_transfer_tuples)
...
train_transfer_data_indices = token2index_dataset(train_data_tokens,token2id)
...
if args.model!='RNN':
    ...
    train_transfer_dataset = YelpDataset(train_transfer_data_indices, train_data_labels)
    train_transfer_loader = torch.utils.data.DataLoader(dataset=train_transfer_dataset,
                                           batch_size=args.batch_size,
                                           collate_fn=yelp_collate_func,
                                           shuffle=True)
else:
...
def test_model(loader, model):
    ...
    if args.model!='RNN':
        ...
        return (100 * correct / total)
print("Starting training Transfer")
j = 0
time_point = []
time_string= []
val_acc_list=[]
...
for epoch in range(num_epochs):
    ...
    if args.model!='RNN':
        for i, (data, lengths, labels) in enumerate(train_transfer_loader):
            model.train()
            data_batch, length_batch, label_batch = data.to(device), lengths.to(device), labels.to(device)
            optimizer.zero_grad()
...
#                import pdb; pdb.set_trace()
                ...
                time_point.append(j)
                j += 1
                train_acc= test_model(train_transfer_loader, model)
